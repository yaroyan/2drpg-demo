...
namespace Yaroyan.SeedWork.DDD4U.Infrastructure.Port.Adapter.Persistence.EventSourcing
{
    public class EventStore : IEventStore
    {
        ...
        string IdentityToString(IEntityId id) => id.Id;
        ...
        public void AppendToStream(IEntityId id, long originalVersion, IEnumerable<IEvent> events)
        {
            ...
            var name = IdentityToString(id);
            var data = events.Select(@event => new StoredEvent(id.Id, @event.Sequence, @event.GetType(), _jsonMapper.Serialize(@event), @event.OccurredOn));
            try
            {
